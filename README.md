...
- **Never puts planning / state / memory folders inside your repositories.** Architecture is **four orthogonal layers**, strictly separated per Claude Code's official memory/settings docs:
  ...
  4. **L4 — Source-of-Truth Rule Package Checkout (pipx feed):** `~/.trae/` → the only place Che's Python package + domain rulebooks + skill source live. Feeds all symlinks of L2 via the installer; Claude Code **never reads this directory directly** — only through the adapter symlinks.
