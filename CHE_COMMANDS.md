...
| `/che-worktree [add|list|show|remove]` | `che_core.worktrees` | **NEW:** Manages worktrees (`~/.che-workspaces/<project-slug>/worktrees/<name>/`). A worktree is the only level that binds a filesystem path, and it requires a git repo. |
| `/che-project [init|list|remove|restore|trash-list]` | `che_core.workspaces` | **NEW:** Manages flat projects (`~/.che-workspaces/<project-slug>/`). A project is identified by an explicit slug and binds no path. |
...
**Agent action:**
1. Run `eval "$(che compute_paths WORKTREE_ROOT SESSION_ID)"` → look for `task_graph.md` at `$CHE_WORKSPACE_SHARED/task_graph.md` (OUTSIDE worktree).
...
- `--purge-stale-locks`: Auto-purges `$CHE_SESSION_DIR/_locks/*.lock.json` from aborted runs — resolve path via `che compute_paths` (asks confirm unless this flag present; NEVER inside worktree).
**Syntax examples:**
```
...
3. Preflight #2: resolve manual_test_plan.md path → (a) `--plan-path` given → use; (b) default (no flag): `eval "$(che compute_paths $WORKTREE_ROOT $SESSION_ID)" && echo "$CHE_WORKSPACE_SHARED/manual_test_plan.md"`; (c) no binding created → AskUserQuestion which option.
...
9. Build final 8-section report according to references/MANUAL_TEST_EXECUTION_REPORT.md → save to `$CHE_SESSION_DIR/reports/MANUAL_TEST_EXECUTION_REPORT.md` (OUTSIDE worktree, `che assert_outside_worktree`).
...
4. **Logging.** Every command execution results in a new entry to `session.md` under `$CHE_SESSION_DIR/` (resolved via `che compute_paths` contract; NEVER inside the Che checkout nested inside a user worktree, e.g. `worktree/.che-ai/` or legacy `worktree/.trae/` — §19.1 MORATORIUM).
