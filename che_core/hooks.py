...
from che_core.registry import registry_lookup_last, get_registry_path
...
def pretooluse_worktree_binding(input_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for p in candidate_paths:
        if str(p).startswith(workspaces_root):
            return {"decision": "allow", "reason": "§19 EXCEÇÃO CHE_SESSIONS_ROOT: path alvo é pasta de dados gerados/efêmeros che."}
    ...
    return {"decision": "allow", "reason": f"§19 OK Level 1 Registry: all detected project paths match BOUND_WORKTREE_ROOT={bound_normalized}"}
...
def posttooluse_lang_pt_check(input_json: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if lang_docs != "en":
        return {"decision": "allow", "reason": f"Hook3 lang-pt: SKIP per session LANG_DOCS={lang_docs} (≠en). sessionId={session_id}."}
    ...
    pt_stopwords = [
        "não", "sim", "que", "de", "do", "da", "dos", "das", "para", "com", "sem", "por", "per",
        "este", "esta", "estes", "estas", "esse", "essa", "esses", "essas", "nós", "você", "vocês",
        "tem", "têm", "são", "foi", "fui", "ser", "estar", "estou", "é", "e", "ou", "mas", "porém",
        "todavia", "contudo", "portanto", "logo", "já", "até", "mais", "menos", "muito", "pouco",
        "hoje", "ontem", "amanhã", "trabalho", "trabalhar", "projeto", "ticket", "tarefa", "sessão",
        "usuário", "arquivo", "código", "erro", "sucesso", "falha", "mudança", "alteração",
        "implementação", "verificação", "validação", "execução", "criar", "criado", "remover",
        "removido", "adicionar", "adicionado", "atualizar", "atualizado", "corrigir", "corrigido",
        "testar", "testado", "aprovar", "aprovado", "rejeitar", "rejeitado", "problema", "solução",
        "resultado", "esperado", "obtido", "função", "método", "classe", "variável", "parâmetro",
...
File analyzed: {file_path}"""
    ...
    return {"decision": "allow", "reason": f"Hook3 lang-pt: OK no PT-BR content detected in {file_path} (stopword_hits={pt_hits} diacritic_lines={diacritic_lines})"}
