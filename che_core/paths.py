...
def resolve_workspace_name(cwd_override: Optional[str] = None) -> str:
    ...
    code_ws_global = Path(os.environ.get("CHE_CODE_WORKSPACES_DIR") or os.environ.get("HARNESS_CODE_WORKSPACES_DIR") or Path.home() / "code" / "code_workspaces")
...
def project_slug_from_git_origin(worktree_root: str) -> str:
    ...
    try:
        res = subprocess.run(
            ["git", "-C", str(wt_path), "remote", "get-url", "origin"],
            capture_output=True, text=True, check=False
        )
        if res.returncode == 0:
    ...
    def assert_outside(candidate_path: Path, label: str):
        try:
            if candidate_path == wt_root or wt_root in candidate_path.parents:
                print(f"🔴 CHE SESSIONS CONTRACT VIOLATION — HARD STOP")
