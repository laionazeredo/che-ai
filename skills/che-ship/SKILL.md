...
3. **IF `effective_domain !== 'engineering':`**
   ...
   c. **For EACH gate file (0.9.5.1, 0.9.5.2, ...):**
      ...
      - Verdict:
        ...
        | 🟡 INCONCLUSIVE | **HARD STOP. NOT a skip, NOT a retry, NOT a pass.** The gate ran and could not measure — so there is no score to retry against, and reporting it as PASS would convert "never verified" into "verified". Read the gate report's `reason` (it names what was unmeasurable), resolve it (missing design reference, category the source does not expose, wrong `--design-backend`), then re-run. Log: `che decision_append "$WORKTREE_ROOT" "DOMAIN-GATE-INCONCLUSIVE" "domain=${effective_domain} gate=${GATE_BASENAME} reason=${REASON} report=${DOMAIN_GATE_REPORT}"`. EXPLICIT_OVERRIDE is NOT valid here: the override exists for a *measured* disagreement, never for a missing measurement. |
