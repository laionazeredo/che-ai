---
name: "harness-manual-test-executor"
description: "Executes the manual_test_plan.md step-by-step using Playwright MCP (browser interactions + screenshots) or HTTP/curl requests for API-only scenarios, generates evidence files (PNG screenshots, JSON logs), and delivers a structured pass/fail report per AC. Call this AFTER manual_test_plan.md is written by harness-scrum-master and BEFORE harness-ship, or standalone when user passes a worktree + task-id or a manual test plan path explicitly."
---

# Harness — Manual Test Executor (step-by-step Playwright + HTTP)

> **SHARED REFERENCES (CANONICAL — NÃO DUPLICAR corpo aqui):**
> - Manual Test Plan canonical format (AC structure with GWT / Manual Steps / Severity): harness-scrum-master `references/MANUAL_TEST_PLAN_TEMPLATE.md`
...
> **Rule:** If user says "rodar testes unitários/E2E" → use `harness-qa`. If user says "executar o plano de testes manual / abrir navegador e testar / checar o comportamento visual" → use THIS skill. Never mix.
