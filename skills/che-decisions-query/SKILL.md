...
**Use when:** User asks to see / filter / summarize / audit decisions from a worktree's `decisions.log.jsonl` (single source of truth JSONL format, v1 schema). Also use when any skill/che skill needs to query decision history programmatically.

**What it does:** Queryable wrapper around `$CHE_WORKSPACE_SHARED/decisions.log.jsonl` for a given worktree. Resolves path via `che_decisions_path` contract. Runs queries via in-process python3/jq — no new dependencies.

**Do NOT use for:** Writing new entries (use `che_append_decision_jsonl` in contracts/ instead, it's the single writer helper with dedup + schema).
...
2. **Source contracts first (STORAGE BOUNDARY PREFLIGHT, MANDATORY mesmo sendo skill só de leitura):
   ...
   # 3. Paths canônicos (para esta sessão (ainda que decisions já resolve che_decisions_path
...
4. **Optional EXPORT para planilha (escritura output opcional — usa che_output_path para garantir ordem timestamp + grouping por PR relacionado):
...
**NOTA:** Skill é read-only por padrão; mas se tiver que escrever export/audit output temporário → helper centralizado acima. Queda cair che-sessions por construção `che_output_path`, NUNCA cai worktree.
