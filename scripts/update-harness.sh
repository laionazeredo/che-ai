#!/usr/bin/env bash
#
# update-harness.sh — Alias INTELIGENTE para atualizar o harness.
#
...
#     → executa: install-harness.sh --update [--apply] --source=<este diretório> --target=$TARGET
#        usando a lógica não-destrutiva descrita em install-harness.sh (backups individuais,
#        blacklist intocável, preserve items custom do target que não existem na source).
#
# USO:
#   ./scripts/update-harness.sh               # DRY-RUN (fetch / install --update dry-run).
#   ./scripts/update-harness.sh --apply       # Aplica a atualização de verdade.
#   ./scripts/update-harness.sh --target /custom/.trae
#   ./scripts/update-harness.sh -h
#
...
#   * CASO 2 reutiliza a lógica fail-closed de install-harness.sh --update (backups
...
# Resolve o diretório onde ESTE script update-harness.sh mora para detectar a SOURCE
...
echo "==> Harness update ${MODE_LABEL}"
echo "    Target: ${TARGET}"
...
if is_git_repo_with_remote "$TARGET"; then
  ...
  if [ "$UNCOMMITTED" -gt 0 ]; then
    ...
    if [ "$APPLY" -eq 1 ]; then
      ...
      echo "    b) Descarte:  cd ${TARGET} && git stash push -m \"wip antes update harness\""
...
# ============================================================
# CASO 2 — NÃO é git repo → fallback para install-harness.sh --update
# ============================================================
echo "ℹ  CASO 2 DETECTADO: target ${TARGET} NÃO é um git repo."
echo "   Estratégia: install-harness.sh --update (não-destrutivo, backups individuais)."
echo ""
echo "   Source usada: ${SOURCE}"
echo ""

# Valida que a source é válida.
if [ ! -f "${SOURCE}/scripts/install-harness.sh" ]; then
  echo "❌ ERRO: Não consegui localizar install-harness.sh em ${SOURCE}/scripts/"
...
echo "    Executando: bash ${SOURCE}/scripts/install-harness.sh $(printf '%q ' "${EXTRA_ARGS[@]}")"
echo ""
bash "${SOURCE}/scripts/install-harness.sh" "${EXTRA_ARGS[@]}"
