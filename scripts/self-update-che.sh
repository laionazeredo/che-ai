#!/usr/bin/env bash
#
# self-update-che.sh — 1 COMANDO para pegar a última versão do che oficial
#                           e mesclar em sua instalação local SEM perder dados pessoais.
#
# PREMISSA (cenário real do usuário):
#   "Pessoa já tem o che instalado em ~/.trae há semanas/meses. Não lembra
...
#   bash ~/.trae/scripts/self-update-che.sh               # DRY-RUN (padrão).
#   bash ~/.trae/scripts/self-update-che.sh --apply       # Aplica de verdade.
...
#          → executa: bash scripts/update-che.sh --[apply] (git pull --ff-only no TARGET MESMO;
...
#          → executa: bash <tmp-src>/scripts/install-che.sh --update [--apply]
...
#
# GARANTIAS FAIL-CLOSED (estendidas de update-che + install-update):
...
#   - `gh` (GitHub CLI) — OBRIGATÓRIO E ÚNICO. Regra hard stop che 2026-09-01:
#     NÃO existe mais fallback git clone HTTPS público. Todos acessos a GitHub no
#     che (PRs, diffs, comments, releases, clone) passam EXCLUSIVAMENTE por gh CLI.
...
if [ ! -f "${TARGET}/README.md" ] && [ ! -f "${TARGET}/CHE_RULES.md" ]; then
  echo "❌ A pasta ${TARGET} não parece ser um .trae de che (sem README.md nem CHE_RULES.md)." >&2
  exit 2
fi
...
if [ -n "${TMP_SRC:-}" ]; then
  ...
  # 2A) gh CLI OBRIGATÓRIO (HARD RULE do che: nenhuma interacao GitHub por HTTP/git clone manual.
  ...
  if ! command -v gh >/dev/null 2>&1; then
    echo ""
    echo "❌ GitHub CLI (gh) NÃO ESTÁ INSTALADO. Che NÃO suporta mais fallback git clone HTTPS público (regra hard stop)." >&2
    echo "   Instale gh CLI + autentique:" >&2
    echo "        https://cli.github.com/" >&2
    echo "        gh auth login --scopes repo,read:org,workflow" >&2
    echo "   Depois rode novamente: bash ~/.trae/scripts/self-update-che.sh" >&2
    exit 6
  fi
...
if [ ! -f "${TMP_SRC}/CHE_RULES.md" ]; then
  echo "❌ Fetch concluído mas a pasta baixada NÃO tem CHE_RULES.md — repo não confiável ou download corrompido." >&2
  exit 8
fi
...
if is_target_git_repo_with_remote "$TARGET"; then
  ...
  # Chamamos update-che.sh do TARGET mesmo (ff-only pull no próprio repo).
  # ==========================================================
  echo "🟢 CASO 1 DETECTADO: target ${TARGET} é um git repo com remoto."
  echo "   Estratégia: git pull --ff-only no próprio TARGET (update-che.sh via source tmp)."
  ...
  # recebe o EXIT da execução (trap é do shell pai). Se update-che c/ exit !=0,
  # trap roda normalmente.
else
  # ==========================================================
  # CASO 2 — target NÃO é git repo (ex: instalou via zip, cópia manual).
  # Chamamos install-che.sh --update usando TMP_SRC como source.
  # ==========================================================
  echo "🟡 CASO 2 DETECTADO: target ${TARGET} NÃO é um git repo."
  echo "   Estratégia: install-che.sh --update (merge não-destrutivo item-a-item)."
