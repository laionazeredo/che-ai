...
- `feat(db): added a new "Refunds" database table + 5 schema update scripts (DDL).` Why: stores every refund request with its Stripe reference and status (Processing → Completed / Failed). Includes RLS (Row-Level Security — Postgres access rules) so each organisation can only see its own refunds.
...
> **Auto-filled when SbE SPEC is approved and `// @ac B-X` anchors are extracted from the diff. If it is a legacy spec (without B-IDs) → REMOVE THIS ENTIRE SECTION in the real PR.**
>
> Each row = a B-ID behavior or AB-ID anti-behavior approved in §4.2 Behavior Table of the SbE spec. Coverage < 70% → see action items from bilateral scope-checker CHECK2.

| Behavior ID (B or AB) | Test file path (extracted from `// @ac B-X` anchors in diff — include :line-range if it is an it() block) |
|---|---|
...
> **Coverage audit rule:** If the SbE §4.2 Behavior Table lists 10 B-IDs and this table only has 6 → coverage = 60% → G7.3 warning in Category 7 of code-review (upgrade to HIGH if ≤2 HIGH gate).
