...
3.  **Worktree Level**: `<project>/worktrees/<worktree-name>/` — the only level that binds a repository path. Born with nothing but `.binding.json`: no artifact folder is pre-created, each appears on its first write via `che output_path` (the single creator of artifact directories — never `mkdir` one by hand). Holds `decisions.log.jsonl` plus the tactical artifacts (`specs/`, `tasks/`, `qa/evidence/`, `reports/`, `pr_plans/`, …) and `.quarantine/` for planning artifacts evicted from a user repository. Shared by every session bound to it.
