---
description: "Gerencia workspaces Che (L1 ~/.che-workspaces/workspaces/<slug>). 5 subcommands: create NAME (alias: add) | list | remove NAME [--dry-run|--no-dry-run --confirm] | restore TRASH_SLUG | trash-list. Workspaces são a camada L1 da hierarquia 4-nível do Che."
arguments:
  - name: subcommand
    description: "Required positional: create <NAME> (alias: add) | list | remove <NAME> [--no-dry-run --confirm] | restore <TRASH_SLUG> | trash-list. Remoção NUNCA apaga: move para .trash/. Dry-run DEFAULT. Exemplos: /che-workspace create flockr / /che-workspace add flockr / /che-workspace list / /che-workspace remove foo / /che-workspace remove foo --no-dry-run --confirm / /che-workspace restore workspace--foo--20260904-235959 / /che-workspace trash-list"
    required: true
---
...
|---|---|---|
| `create <NAME> [--worktree-root WT]` | `python3 -m che_core.cli workspace create "<NAME>" [--worktree-root "..."] --json` | Cria pasta `~/.che-workspaces/workspaces/<slug>/` + `_MANIFEST.json`. Se `--worktree-root` foi passado → workspace é marcado como "default" para esse worktree. Report `created=True`, slug e path absoluto. |
| `add <NAME>` | `python3 -m che_core.cli workspace add "<NAME>" --json` | Alias para `create`. Mantido para compatibilidade. |
