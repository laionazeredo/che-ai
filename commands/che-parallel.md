...
2. If any task envelope Blast Radius still has globs (no explicit file list) → ERROR. User MUST enumerate files — NO serial fallback (parallel-or-bust when command is explicitly /che-parallel).
3. Then invoke **`che-executor-dispatcher`** to fan out.
