---
description: "Read and summarize entries from worktree decisions.log.jsonl (Portuguese summary). Use Skill harness-decisions-query for filters/export."
arguments:
  - name: worktree
...
Lightweight inline command. Resolve path via contracts helper. Use Skill `harness-decisions-query` for advanced queries (date ranges, export options, grep keyword, csv full).
...
2. Source contracts + resolve path:
   ...
   JSONL=$(harness_decisions_path "<WORKTREE_ROOT>")
   ```
...
4. Default mode = summary. Run CLI helper TypeScript:
   ...
   ```
5. If mode=filter/event/spec → invoke Skill `harness-decisions-query` to do the filtering.
